...
def model(tensor_in, is_training):
    ...
    for i in range(4):
        ...
        cnn_result = cnn_layer(last_layer, filtersizes, filternums,
                               ...
                               constraint=mastl.parsnet.constraints.tight_frame(0.001),
                               scale_filter=True)
...
optimistic_restore(sess, "trained_parseval/last_run.ckpt")
...
for i in range(2, 6):
    ...
    np.save("results_parseval/{}_reconstruction_original.npy".format(i), recon_original)
    np.save("results_parseval/{}_reconstruction_perturbation.npy".format(i), recon_pert)
