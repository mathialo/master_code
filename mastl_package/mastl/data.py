from mastl import _iohelp as io
...
class DataSet(Sized):
    ...
    def __init__(self, directory, namepattern, fileloader, data_key=None, label_key=None,
                 scale=False, cache=False, cache_limit=512, augment=False, crop=None, test_rate=0):
        """
        ...
        Args:
            ...
            cache_limit (int):      Number of MB to leave in memory when caching
        ...
        self.cache_threshold = cache_limit # mb
    ...
    def _enough_memory(self):
        if self.stop_cache:
            ...
            if psutil.virtual_memory().available < self.cache_threshold*1e6:
                print()
                io.log("Less than {} mb left in memory, DataSet will not cache any more files".format(self.cache_threshold), "warn")
